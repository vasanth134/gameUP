  ...
  try {
    ...
    if (status === 'approved') {
      ...
      const taskResult = await pool.query(
        `SELECT t.xp_reward, t.child_id, t.title, u.name as child_name, u.total_xp
         FROM tasks t 
      ...
      if (task) {
        ...
        await pool.query(
          ...
          [newXPTotal, task.child_id]
        );

        // Send success notification
        await pool.query(
          `INSERT INTO notifications (child_id, message, created_at)
           VALUES ($1, $2, NOW())`,
          [task.child_id, `🎉 Congratulations! Your task "${task.title}" has been approved! You earned ${xpAwarded} XP points!`]
        );
      ...
      const taskResult = await pool.query(
        `SELECT t.title, t.child_id, u.name as child_name
         FROM tasks t 
         JOIN users u ON t.child_id = u.id
         WHERE t.id = $1`,
        [updatedSubmission.task_id]
      ...
      if (task) {
        ...
        await pool.query(
          `INSERT INTO notifications (child_id, message, created_at)
           VALUES ($1, $2, NOW())`,
          [task.child_id, rejectionMessage]
        );
...
export const createSimpleSubmission = async (req: Request, res: Response): Promise<void> => {
  try {
    const { task_id, child_id, submission_text } = req.body;
...
export const getAllSubmissionsForParent = async (req: Request, res: Response): Promise<void> => {
  const { parentId } = req.params;
